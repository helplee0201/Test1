...
def get_checklist_data():
    ...
    checklist_data = [
        ...
        {"구분": "데이터", "유형": "최신성", "항목": "1) 1분기 데이터는 동년 기준월이 4월 이후인 시점부터 매월 변동성을 집계함"},
        {"구분": "데이터", "유형": "최신성", "항목": "2) 2분기 데이터는 동년 기준월이 7월 이후인 시점부터 매월 변동성을 집계함"},
        {"구분": "데이터", "유형": "최신성", "항목": "3) 3분기 데이터는 동년 기준월이 10월 이후인 시점부터 매월 변동성을 체크함"},
        {"구분": "데이터", "유형": "최신성", "항목": "4) 4분기 데이터는 다음해 기준월이 1월 이후인 시점부터 매월 변동성을 체크함"},
        {"구분": "데이터", "유형": "무결성", "항목": "테크핀은 시스템날짜 기준으로 2개월 전 기장 내역을 제공하고 있으며 (일부 1개월 전) 월별 사업자수를 집계하여 최신 데이터 제공 여부를 판단함"},
        {"구분": "데이터", "유형": "무결성", "항목": "1. 복수개의 사업자번호(NO_BIZ)에 대한 기장데이터 중에서 1개의 NO_COM을 선택한 후 7개 테이블 산출 로직을 적용함"},
        {"구분": "데이터", "유형": "무결성", "항목": "2) 과거에 산출한 결과값과 당월 산출한 결과값이 다른 원인을 분석함"},
        {"구분": "데이터", "유형": "무결성", "항목": "3) 동일 회계기수를 넘어 선 과거 데이터에 대한 변동이 서로 다른 NO_COM 선택으로 인한 문제인지를 확인함"},
        {"구분": "데이터", "유형": "무결성", "항목": "4) 원본 데이터에 대한 값 변동이 아닌 클렌징로직에 따른 변경인 경우 과거 데이터에 대한 클렌징로직 적용을 제한하는 방안을 검토해야 함"},
    ...
    for type_value in unique_types:
        ...
        for idx, item in enumerate(type_data):
            if item.startswith(tuple(str(i) + "." for i in range(1, 10))):  # "1.", "2.", ... "9."로 시작하는 항목에만 체크박스
                ...
                st.write(item)  # 체크박스 없는 항목은 plain text로 표시
        
        # 체크된 항목 수 계산 및 업데이트 (numbered items만 카운트)
        checked_count = sum(1 for checked in checked_items if checked)
        st.session_state.checked_counts[type_value] = checked_count
        
        # 체크된 항목 수 표시
        st.write(f"체크된 항목 수: {checked_count} / {len([item for item in type_data if item.startswith(tuple(str(i) + '.' for i in range(1, 10)))])}")
